...
internal fun DocumentSnapshot.toTaskEntity(): TaskEntity? {
    ...
    return TaskEntity(
        ...
        priority = (data["priority"] as? String)?.let { TaskPriority.fromName(it) } ?: TaskPriority.MEDIUM,
        status = (data["status"] as? String)?.let { TaskStatus.fromName(it) } ?: TaskStatus.PENDING,
...
internal fun DocumentSnapshot.toProgressEntity(): ProgressEntity? {
    ...
    return ProgressEntity(
        ...
        examType = (data["examType"] as? String)?.let { ExamType.fromName(it) } ?: ExamType.JEE_MAIN,
        chapterState = (data["chapterState"] as? String)?.let { ChapterState.fromName(it) } ?: ChapterState.NOT_STARTED,
