...
fun ErrorEditScreen(
    ...
    errorId: String?,
    vm: ErrorEditViewModel = viewModel(),
) {
    val ui by vm.ui.collectAsStateWithLifecycle()
    LaunchedEffect(errorId) { vm.load(errorId) }
    LaunchedEffect(ui.saved) {
        if (ui.saved) nav.popBackStack()
    }

    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(if (errorId == null) "Add Error" else "Edit Error") },
                navigationIcon = {
        ...
        Column(
            ...
            DropdownField(
                ...
                options = Subject.entries.map { it.displayName },
                onSelected = { name -> vm.update { it.copy(subject = Subject.fromName(name)) } },
            )
            ...
            DropdownField(
                label = "Source",
                value = ui.source.displayName,
                options = QuestionSource.entries.map { it.displayName },
                onSelected = { name -> vm.update { it.copy(source = QuestionSource.fromLabel(name)) } },
            )
            ...
            DropdownField(label = "Mistake Type", value = ui.mistakeType.label, options = MistakeType.entries.map { it.label }, onSelected = { l -> vm.update { it.copy(mistakeType = MistakeType.fromLabel(l)) } })

            OutlinedTextField(value = ui.attemptedSolution, onValueChange = { v -> vm.update { it.copy(attemptedSolution = v) } }, label = { Text("Attempted solution") }, modifier = Modifier.fillMaxWidth().heightIn(min = 80.dp))
            OutlinedTextField(value = ui.correctSolution, onValueChange = { v -> vm.update { it.copy(correctSolution = v) } }, label = { Text("Correct solution") }, modifier = Modifier.fillMaxWidth().heightIn(min = 80.dp))
            OutlinedTextField(value = ui.explanation, onValueChange = { v -> vm.update { it.copy(explanation = v) } }, label = { Text("Explanation") }, modifier = Modifier.fillMaxWidth().heightIn(min = 80.dp))
            OutlinedTextField(value = ui.lessonLearned, onValueChange = { v -> vm.update { it.copy(lessonLearned = v) } }, label = { Text("Lesson learned") }, modifier = Modifier.fillMaxWidth().heightIn(min = 80.dp))
